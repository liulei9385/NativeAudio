...
public class LyricView extends TextView {
    ...
    private ArrayList<LyricBean> lyrics = new ArrayList<>();
    ...
    private void initLyricInfo(AttributeSet attrs) {
        ...
        defaultLyricSize = ta.getDimension(R.styleable.LrcView_lrcTextSize, DensityUtils.sp2px(getContext(), 14));
        ...
        ta.recycle();
        // 初始化其他相关的值
        currentLyricSize = defaultLyricSize + DensityUtils.sp2px(getContext(), 3.0f);
        colors[0] = mCurrentColor;
        colors[1] = mGradientEndColor;
        defaultLineHight = DensityUtils.dp2px(getContext(), 24.0f);
        paint = new Paint();
        // 设置字体边缘平滑
    ...
    protected void onDraw(Canvas canvas) {
        if (CollectionUtils.isNotEmpty(lyrics)) {
    ...
    public void updateCurrentLyricIndex(long currentPosition) {
        // 遍历集合
        for (int i = 0; i < lyrics.size(); i++) {
     ...
     **/
    public void updateLyrics(long currentPosition, long duration) {
        if (this.duration == 0)
    ...
    private float smoothScrollLyric() {
        // 如果是最后一个行 处理越界问题
        // 获取当前歌词的时间
        long time = lyrics.get(currentLyricIndex).time;
        long totalTime;
        ...
        // 获取当前歌词已经唱了多久
        long passedTime = currentPosition - time;
