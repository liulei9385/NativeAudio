...
public interface KugouLyriService {
    ...
    @GET("search?ver=1&man=no&client=android&keyword={songname}&duration={duration}&hash=")
    Observable<KugouLyricRecord> getLryicRecord(@Path("songname") String songname, @Path("duration") long duration);

    // notice: 2016/12/5
    //download?ver=1&client=pc&id=10515303&accesskey=3A20F6A1933DE370EBA0187297F5477D&fmt=lrc&charset=utf8 （fmt=lrc 或 krc）
    @GET("download?ver=1&client=android&id={id}&accesskey={accesskey}&fmt=lrc&charset=utf8")
    Observable<KugouLyricPath> downloadLryic(@Path("id") String id, @Path("accesskey") String accesskey);
